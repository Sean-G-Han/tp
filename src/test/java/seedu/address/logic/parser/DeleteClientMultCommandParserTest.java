 ...
 * outside of the DeleteClientMultCommand code. For example, inputs "1" and "1 abc" take the
...
public class DeleteClientMultCommandParserTest {
    ...
    public void parse_validArgs_returnsDeleteClientMultCommand() {
        // Single index
        assertParseSuccess(parser, "1", new DeleteClientMultCommand(Arrays.asList(INDEX_FIRST_CLIENT)));

        // Multiple indices
        List<Index> indices = Arrays.asList(INDEX_FIRST_CLIENT, INDEX_SECOND_CLIENT, INDEX_THIRD_CLIENT);
        assertParseSuccess(parser, "1 2 3", new DeleteClientMultCommand(indices));
    }
